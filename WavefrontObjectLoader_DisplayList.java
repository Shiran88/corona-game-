...
public class WavefrontObjectLoader_DisplayList {
    ...
    public static List<Float> getSurroundingPoints() {
        List<float[]> vData = recentlyLoaded.getvData();
        float xMax = 0;
        float xMin = 0;
        float zMax = 0;
        float zMin = 0;
...
//        points.add(new float[]{xMin, zMin});
//        points.add(new float[]{xMax, zMin});
//        points.add(new float[]{xMax, zMax});
//        points.add(new float[]{xMin, zMax});
        return Arrays.asList(xMin,xMax,zMin,zMax);
    }

    public static int getAndCleanLastLoaded(GL2 inGL) {
        if (recentlyLoaded != null) {
            recentlyLoaded.drawModel(inGL);
            inGL.glEndList();
        }
        return recentlyLoadedDisplayListID;

    }

    private static WavefrontObjectLoader_DisplayList recentlyLoaded;
    private static int recentlyLoadedDisplayListID;
    // Caching already loaded models, so that it would not reload the same model twice.
    static Map<String, Integer> fileToID = new HashMap<>();
//    static Map<String, WavefrontObjectLoader_DisplayList> fileToLoader = new HashMap<>();
    public static void loadWavefrontObjectAsDisplayList(GL2 inGL,String inFileName) {
        if (fileToID.containsKey(inFileName)) {
            recentlyLoadedDisplayListID = fileToID.get(inFileName);
            recentlyLoaded = null;
        } else {
            recentlyLoadedDisplayListID = inGL.glGenLists(1);
            recentlyLoaded = new WavefrontObjectLoader_DisplayList(inFileName);
            inGL.glNewList(recentlyLoadedDisplayListID, GL_COMPILE);
            fileToID.put(inFileName,recentlyLoadedDisplayListID);
//            fileToLoader.put(inFileName,recentlyLoaded);
        }
